...
import { isBloomStyleFillable } from '@/lib/fillSource'
import {
  CenterSliderRow,
  LockedLayerBanner,
  LogoFillPicker,
...
export function BloomPanel({ layerId }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Inner Fill — only shown on closed-area styles. Echo and
          MultiRing have no enclosed area so the fill would be a
          no-op even if configured; hiding the UI keeps the panel
          honest. The fill config persists across style switches
          (we never null the fields here) so hopping to a non-
          fillable style and back restores the previous setup. */}
      {isBloomStyleFillable(cfg.style) && (
              ...
              {videoAssets.length === 0 ? (
                ...
                <>
                  <select
                    ...
                    aria-label="Bloom video fill source"
                  >
